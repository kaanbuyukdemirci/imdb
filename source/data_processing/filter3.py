...
@beartype
def filter3(chunk_size:int=10**6)->None:
    """
    from
    ...
    average rating is the imdb rating of the titleId
    """
    
    # read and write
    read_location_1 = ".\\data\\raw\\"
    read_name_1 = "title.ratings.tsv.gz"
    read_location_2 = ".\\data\\processed\\"
    read_name_2 = "filter2_output.csv"
    write_location = ".\\data\\processed\\"
    write_name = "filter3_output.csv"
    
    # read the first file
    with pd.read_csv(filepath_or_buffer=read_location_1+read_name_1, sep='\t', header=0, index_col='tconst', 
                     usecols=['tconst', 'averageRating'], na_values=['\\N'], chunksize=chunk_size, compression='infer',
        ...
        for chunk_1 in context_manager_1:
            ...
            # read the second file so that you can merge two files together
