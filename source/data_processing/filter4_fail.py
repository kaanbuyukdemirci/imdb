...
def filter4(chunk_size:int=10**6)->None:
    ...
    I tried to filter the movies by the 'language' and 'isOriginal'.
    Basically, if it is the original title and it is english, keep the movie, otherwise eliminate.
    However, it didn't work because the data is incomplete.
    See the output for this filter.
    That is why I took a different approach in filter4.py and the following filters.
    """
    ...
    write_name = "filter4_fail_output.csv"
    
    # read the first file
    with pd.read_csv(filepath_or_buffer=read_location_1+read_name_1, sep='\t', header=0, index_col='titleId', 
                     usecols=['titleId', 'language', 'isOriginalTitle'], na_values=['\\N'], chunksize=chunk_size, compression='infer',
        ...
        for chunk_1 in context_manager_1:
            ...
            chunk_1 = chunk_1.dropna(axis='index', how='any', inplace=False).astype({'isOriginalTitle':np.int16}).astype({'isOriginalTitle':np.bool_})
            ...
            filt_2 = chunk_1['isOriginalTitle'] == np.bool_(True)
            chunk_1 = chunk_1[filt_1&filt_2]
